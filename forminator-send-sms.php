<?php

/**
 * The plugin bootstrap file
 *
 * This file is read by WordPress to generate the plugin information in the plugin
 * admin area. This file also includes all of the dependencies used by the plugin,
 * registers the activation and deactivation functions, and defines a function
 * that starts the plugin.
 *
 * @link              https://mwale.me
 * @since             1.0.0
 * @package           Forminator_Send_Sms
 *
 * @wordpress-plugin
 * Plugin Name:       Forminator Send SMS
 * Plugin URI:        https://mwale.me
 * Description:       Instantly send custom form data via BulkSMS API.
 * Version:           1.0.0
 * Author:            Mwale Kalenga
 * Author URI:        https://mwale.me
 * License:           GPL-2.0+
 * License URI:       http://www.gnu.org/licenses/gpl-2.0.txt
 * Text Domain:       forminator-send-sms
 * Domain Path:       /languages
 */

require_once __DIR__ . '/vendor/autoload.php';
$dotenv = Dotenv\Dotenv::createImmutable(__DIR__);
$dotenv->safeLoad();

// If this file is called directly, abort.
if ( ! defined( 'WPINC' ) ) {
	die;
}

/**
 * Currently plugin version.
 * Start at version 1.0.0 and use SemVer - https://semver.org
 * Rename this for your plugin and update it as you release new versions.
 */
define( 'FORMINATOR_SEND_SMS_VERSION', '1.0.0' );

 /*
* The environment and accesss configurations
*/

$config = array(
    'username' => $_ENV['Username'],
    'password' => $_ENV['Password'],
);

/**
 * The code that runs during plugin activation.
 * This action is documented in includes/class-forminator-send-sms-activator.php
 */
function activate_forminator_send_sms() {
	require_once plugin_dir_path( __FILE__ ) . 'includes/class-forminator-send-sms-activator.php';
	require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
	Forminator_Send_Sms_Activator::activate();
}

/**
 * The code that runs during plugin deactivation.
 * This action is documented in includes/class-forminator-send-sms-deactivator.php
 */
function deactivate_forminator_send_sms() {
	require_once plugin_dir_path( __FILE__ ) . 'includes/class-forminator-send-sms-deactivator.php';
	Forminator_Send_Sms_Deactivator::deactivate();
}

register_activation_hook( __FILE__, 'activate_forminator_send_sms' );
register_deactivation_hook( __FILE__, 'deactivate_forminator_send_sms' );

/**
 * The core plugin class that is used to define internationalization,
 * admin-specific hooks, and public-facing site hooks.
 */
require plugin_dir_path( __FILE__ ) . 'includes/class-forminator-send-sms.php';

/**
 * Begins execution of the plugin.
 *
 * Since everything within the plugin is registered via hooks,
 * then kicking off the plugin from this point in the file does
 * not affect the page life cycle.
 *
 * @since    1.0.0
 */
function run_forminator_send_sms($config) {

	$plugin = new Forminator_Send_Sms($config);
	$plugin->run();

}
run_forminator_send_sms($config);